...
filepath <-
  "/Users/macbook/Dropbox/University of Bonn/PhD Projects/Negusse/02_Spate_irrigation/Spate/"
...
make_variables(estimate_read_csv(paste(
  filepath, "Turkana_estimates_190313.csv", sep = ""
)))
...
spate_model <- function(x, varnames)
{
  ...
  cost_conflict_management <-
    c(
      0,
      chance_event(
        prob_conflict,
        value_if = cost_managing_conflict,
        value_if_not = 0,
        n = n_years - 1,
        CV_if = general_CV
      )
  ...
  drought <-
    1 - chance_event(
      prob_yield_red_dry,
      value_if = perc_reduction_crop_yield / 100,
      value_if_not = 0,
      n = n_years,
      CV_if = general_CV
    )
  ...
  yield_reduction_flooding <-
    1 - chance_event(
      prob_yield_red_flood,
      value_if = perc_reduction_crop_yield_flood / 100,
      value_if_not = 0,
      n = n_years,
      CV_if = general_CV
    )
  ...
  community_awareness_mobilization_cost <-
    c(training_cost, rep(0, n_years - 1))
  ...
  watershed_management_cost <-
    c(
      watershed_area * perc_watershed_area_intervened / 100 * cost_watershed_mgmt,
      rep(0, n_years - 1)
    )
  ...
  total_repair_maintenace_budget <-
    (construction_cost_stru[1] * perc_repair_maint_cost / 100) / (n_years -
                                                                    1)
  annual_repair_cost <-
    c(0, rep(total_repair_maintenace_budget, n_years - 1))
  ...
  total_implementers_cost <-
    construction_cost_stru + community_awareness_mobilization_cost +
  ...
  cost_family_labour <-
    c(0, vv(family_labour_cost, general_CV, n_years - 1))
  commun_structural_rm_contribution <-
    c(0, (
      vv(labour_contribution, general_CV, n_years - 1) * cost_family_labour[2:n_years]
    )) *
  ...
  cost_reduction_alluvial_deposite <-
    c(0, (
      vv(area_benefited_alluvial_deposits, general_CV, n_years - 1) *
        vv(value_alluvial_deposits, general_CV, n_years -
             1)
  ...
  human_health_cost <-
    c(0, (
      vv(no_people_affected, general_CV, n_years - 1) * vv(cost_health_treatment, general_CV, n_years -
                                                             1)
    )) *
    (1 - failure_due_to_poor_implementation)
  
  # Cost 11: cost of lossing the current pasture production by shifting to crop production
  value_pasture_without_interv <-
    vv(current_value_pasture, general_CV, n_years)
  total_farming_area <-
    c(0, vv(total_area, general_CV, n_years - 1)) * (1 - failure_due_to_poor_implementation)
  
  value_pasture_lost <-
    total_farming_area * prop_area_converted_crop * value_pasture_without_interv
  # Total communal cost
  total_communal_costs <-
    commun_structural_rm_contribution + cost_reduction_alluvial_deposite + human_health_cost +
    value_pasture_lost
  ...
  crop_productivity <-
    c(0, vv(crop_production, general_CV, n_years - 1, lower_limit = 0))
  crop_residue <-
    c(0, vv(crop_residue, general_CV, n_years - 1, lower_limit = 0))
  
  farm_reveneu <-
    (total_farming_area * ((crop_productivity * price_crop) + (crop_residue *
                                                                 price_residue)
    )) *
  ...
  farm_operational_cost <-
    total_farming_area * ((farm_job_created * cost_family_labour) + other_farm_cost)
  ...
  pasture_area_clearing_cost <-
    total_farming_area * prop_area_converted_crop * clearing_cost_pasture
  ...
  perimeter_fencing_cost[c(2, 7, 12, 17, 22)] <-
    perimeter_fenced * cost_fence
  
  #net farm profit
  farm_profit <-
    farm_reveneu - farm_operational_cost - pasture_area_clearing_cost - perimeter_fencing_cost
  ...
  total_beneficieries <-
    round (total_population * prop_trained * prop_adopted)
  
  capacity_development_benefit <-
    c(total_beneficieries * value_capacity_development,
      rep(0, n_years - 1))
  ...
  farm_job_benefit <-
    total_farming_area * farm_job_created * cost_family_labour
  
  # off farm job opportunity
  off_farm_cost_labour <-
    vv(off_farm_labour_cost, general_CV, n_years)
  
  off_farm_job_benefit <-
    vv(off_farm_job, general_CV, n_years) * off_farm_cost_labour * (1 - failure_due_to_poor_implementation)
  
  #construction job opportunity
  perc_job_construction_work <-
    c(construction_job_perc / 100, rep(0, n_years - 1))
  
  construction_job_benefit <-
    perc_job_construction_work * construction_cost_stru
  
  #watershed management job
  watershed_management_job_created <-
    c(watershed_mgmt_job, rep(0, n_years - 1))
  
  watershed_management_job_benefit <-
    watershed_management_job_created * watershed_area *
  ...
  total_employment_benefit <-
    farm_job_benefit + off_farm_job_benefit + construction_job_benefit +
    watershed_management_job_benefit + repair_maintenance_job
  
  ## benefit 4: benefit reducing flooding effect
  Occurance_flooding_hazard <-
    chance_event(
      prob_flooding_hazard,
      value_if = 1,
      value_if_not = 0,
      n = n_years
    )
  flood_hazard_reduction <-
    Occurance_flooding_hazard * c(0, vv(value_red_flooding_effect, general_CV, n_years -
                                          1)) *
    (1 - failure_due_to_poor_implementation)
  
  ## total other complementary communal benefits (i.e. the sum of all benefits)
  
  total_communal_benefit <-
    farm_profit + capacity_development_benefit + total_employment_benefit +
    flood_hazard_reduction
  ...
  perc_annual_increase_veg_cov_density[perc_annual_increase_veg_cov_density >
                                         1] <- 1
  
  perc_annual_area_reclaimed <-
    cumsum(c(
      0,
      vv(
        perc_area_reclaimed / 100,
        general_CV,
        n_years - 1,
        lower_limit = 0,
        upper_limit = 100
  ...
  vegetative_cover_value <-
    vv(value_vegetative_cover, general_CV, n_years)
  area_reclaimed_value <-
    vv(value_reclaimed_area, general_CV, n_years)
  
  value_increase_vegetative_cover <-
    perc_annual_increase_veg_cov_density * watershed_area * perc_watershed_area_intervened /
    100 *
    vegetative_cover_value
  
  value_increase_area_reclamation <-
    perc_annual_area_reclaimed * watershed_area * perc_watershed_area_intervened /
    100 *
    area_reclaimed_value
  
  # improve in micro climate and ecosystem due to catchment management
  value_improvement_micro_climate <-
    vv(value_improve_ecosystem, general_CV, n_years)
  
  value_improvement_micro_climate <-
    watershed_area * perc_watershed_area_intervened / 100 * value_improvement_micro_climate *
    perc_annual_increase_veg_cov_density
  
  ##total environmental benefit of the proposed intervention
  total_environmental_benfit <-
    value_improvement_micro_climate + value_increase_vegetative_cover + value_increase_area_reclamation
  ...
  veg_forest_area_reduction_cost <-
    c(0,
      rep(
        vegetative_area_affected * value_vegetative_cover / 100,
        n_years - 1
      )) * (1 - failure_due_to_poor_implementation)
  ...
  add_cost_soil_disturbance_emission <-
    c(0, vv(value_soil_disturbance, general_CV, n_years - 1))
  
  cost_soil_disturbance_emission <-
    total_farming_area * soil_exposed_perc * add_cost_soil_disturbance_emission
  ...
  deforestation_cost[c(2, 7, 12, 17, 22)] <-
    perimeter_fenced * value_tree_cleared
  
  # total environmental cost of the proposed intervention
  total_environmental_cost <-
    veg_forest_area_reduction_cost + cost_soil_disturbance_emission + deforestation_cost
  ...
  communal_effect_cash_flow <-
    (total_communal_benefit - total_communal_costs) / exchange_rate
  ...
  income_per_farmer_cash_flow <-
    communal_effect_cash_flow / total_beneficieries
  income_per_farmer_cash_flow <-
    unlist(sapply(income_per_farmer_cash_flow, function(x)
      if (x == 0)
        rnorm(n = 1, 0.1, 0.0001)
      else
        x))
  
  # Net environmental effect
  environmental_effect_cash_flow <-
    (total_environmental_benfit - total_environmental_cost) / exchange_rate
  environmental_effect_cash_flow <-
    unlist(sapply(environmental_effect_cash_flow, function(x)
      if (x == 0)
        rnorm(n = 1, 0.1, 0.0001)
      else
        x))
  ...
  implementer_cash_flow <-
    unlist(sapply(implementer_cash_flow, function(x)
      if (x == 0)
        rnorm(n = 1, 0.1, 0.0001)
      else
        x))
  ...
  overall_project_effect_cash_flow <-
    communal_effect_cash_flow + environmental_effect_cash_flow + implementer_cash_flow
  
  # Net overall project effect (discounted)
  overall_project_effect_NPV <-
    sum(NPV(communal_effect_cash_flow, discount_rate_farmers, TRUE)) + sum(NPV(
      environmental_effect_cash_flow,
      discount_rate_farmers,
      TRUE
    )) +
  ...
  return(
    list(
      Communal_effect_NPV = sum(
        NPV(communal_effect_cash_flow, discount_rate_farmers, TRUE)
      ),
      Income_per_farmer_NPV = sum(
        NPV(income_per_farmer_cash_flow, discount_rate_farmers, TRUE)
      ),
      Environmental_effect_NPV = sum(
        NPV(
          environmental_effect_cash_flow,
          discount_rate_farmers,
          TRUE
      ...
      Implementer_NPV = sum(NPV(
        implementer_cash_flow, discount_rate, TRUE
      )),
...
decisionSupport(
  paste(filepath, "Turkana_estimates_190313.csv", sep = ""),
  #input file with estimates
  paste(filepath, "MCResults", sep = ""),
  #output folder
  write_table = TRUE,
  spate_model,
  10000,
...
base_folder <-
  paste(filepath, "MCResults/", sep = "")   #the folder where your inputs are
out_folder <-
  paste(filepath, "DAresults/", sep = "")   #the folder where your outputs should go
dir.create(out_folder)
MC_file <-
  read.csv(paste(base_folder, "mcSimulationResults.csv", sep = ""))
outvars <- list(
  "Communal_effect_NPV",
...
legendtable <-
  read.csv(paste(filepath, "Turkana_legend.csv", sep = "")) # this one has to be prepared manually

for (i in seq_along(outvars))
{
  makeMCPlot(
    NPV_table = MC_file,
    variable_names = outvars[i],
    colorscheme = "minus_plus_red_green",
    fonttype = "serif",
    lwd = 2,
    cex.axis = 2,
    x_label = paste("Simulated", labels[i], sep = ""),
    borderlines = TRUE,
    scaler = "auto",
    fileformat = "png",
  ...
  PLStab <-
    read.csv(paste(base_folder, outvars[i], "_pls_results.csv", sep = ""))
  
  makePLSplot(
    PLS_table = PLStab,
    npls = 7,
    plot_title = paste(labels[i], sep = ""),
    cex.axis = 1.5,
    lwd = 0.1,
    legend_table = legendtable,
    fileformat = "png",
    filename = paste(out_folder, "PLSplot_", labels[i], ".png", sep = "")
  )
}


mc_EVPI <-
  MC_file[, -grep("cashflow", colnames(MC_file), ignore.case = TRUE)]
...
{
  compound_figure(
    variable_name = outvar,
    MC_table = MC_file,
    PLS_table = read.csv(paste(
      base_folder, outvar, "_pls_results.csv", sep = ""
    )),
    EVPI_table = read.csv(paste(
      out_folder, "EVPI_table_", outvar, ".csv", sep = ""
    )),
    cash_flow_vars = paste("Cashflow_", outvar, sep = ""),
    nbreaks = 100,
    scaler = "auto",
    percentile_remove = c(.01, .99),
    npls = 15,
    plsthreshold = 0.8,
    colorscheme = "quant_col",
    MCcolor = "negpos",
    fonttype = 'sans',
    borderlines = FALSE,
    lwd = 1,
    fileformat = "png",
    filename = paste(filepath, "Compound figures/", outvar, sep = ""),
    legend_table = legendtable
  )
}
